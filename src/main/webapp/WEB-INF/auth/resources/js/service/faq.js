jq(document).ready(function () {
	...
	jq(".sub_service_menu_4").addClass("active").addClass("ov");
...
function doCategoryRows() {
	...
	jq.ajax({
		...
		success: function (res, status, xhr) {
			...
			if (res.returnCode == 0) {
				...
				if (res.data.category[0].length > 0) {
					serviceFAQCategoryIdx = res.data.category[0][0].ServiceFAQCategoryIdx;
		...
		complete: function (xhr, status) {
			...
			if (serviceFAQCategoryIdx != '') {
				categoryClick(serviceFAQCategoryIdx, '1');
			}
		}
	});
 ...
 * Date              Auth        Desc
 */
function categoryClick(serviceFAQCategoryIdx, page) {

	// url: 요청이 전송되는 URL (프로세스명은 뒤에 꼭 _prc 가 붙여져야 함)
	//       Ex) /폴더명/Controllers 폴더 안 php 파일명(컨트롤러명)/실행할 메소드명(프로세스)
	jq.ajax({
		url: "/service/faq/faqList_prc",
		type: "get",
		dataType: "json",
		data : {
			serviceFAQCategoryIdx: serviceFAQCategoryIdx,
			page: page
		},
		success: function (res, status, xhr) {
			// 프레임워크 에러코드가 0 (성공)인 경우
