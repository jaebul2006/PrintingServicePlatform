...
function doDetailRows() {
	...
	jq.ajax({
		url: "/software/qna/qnaDetail_prc",
		type: "get",
		dataType: "json",
		data : {
			swQnAIdx: swQnAIdx
...
function doDelete() {
	...
	var swQnAIdx = jq("#sw_qna_idx").val();
	var returnCode = 1;
	if (swQnAIdx == '' || swQnAIdx == null) {
		alert('서비스 Q&A 번호가 없습니다.');
		return;
	}
	if (confirm('삭제하시면 다시 복구 할 수 없습니다.\n삭제하시겠습니까?')) {
		jq.ajax({
			url: "/software/qna/qnaDelete_prc",
			type: "post",
			dataType: "json",
			data : {
				swQnAIdx: swQnAIdx
...
function doModify() {
	var swQnAIdx = jq("#sw_qna_idx").val();
	location.href = '/software/qna/qnaModify?swQnAIdx='+swQnAIdx+'&page='+jq("#page").val();
}
