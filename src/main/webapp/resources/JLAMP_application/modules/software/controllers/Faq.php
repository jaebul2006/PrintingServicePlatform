...
class Faq extends JLAMP_Controller {
	 ...
	 * Date              Auth        Desc
	*/
	public function softwareFaq() {
		//$returnURL = $this->jlamp_comm->xssInput('returnURL', 'get');		// 권한이 없는 이전 페이지 경로

		$cssPart = array(
			'<link rel="stylesheet" href="/css/software/faq.css">'
		);
		$jsPart = array(
			'<script src="/js/software/faq.js"></script>'
		);
	...
	*/
	public function categoryFaq_prc() {
		...
		$column = ' SWFAQCategoryIdx, Adminidx, Category '; // 검색할 Columns
		$filter = ''; // 조건절
		$sort = ''; // 정렬 기준
		$group = ''; // 그룹 기준
		...
		try {
			...
			// Table 설정
			$this->jlamp_common_mdl->setTable('SWFAQCategory');
			...
			if ($res['rows']) {
				...
				foreach ($res['rows'][0] as $key => $val) {
					if ($i == 0) {
						$categoryHtml .=
							'
							<li class="active" style="border-left: 1px solid #d1d1d1;"><a class="category" style="cursor:pointer;" onclick="categoryClick(\''.$val->SWFAQCategoryIdx.'\',\'1\')">'.$val->Category.'</a></li>';
					} else {
						$categoryHtml .=
							'
							<li><a style="cursor:pointer;" class="category" onclick="categoryClick(\''.$val->SWFAQCategoryIdx.'\',\'1\')">'.$val->Category.'</a></li>';
					}
					$i++;
				}
	...
	*/
	public function faqList_prc() {
		...
		$softwareFAQCategoryIdx = $this->jlamp_comm->xssInput ( 'softwareFAQCategoryIdx', 'get' ); // FAQ 카테고리 번호
		...
		$column = ' SWFAQIdx,SWFAQCategoryIdx, Adminidx, Subject, Content '; // 검색할 Columns
		$filter = array(
            array (
				'SWFAQCategoryIdx' => $softwareFAQCategoryIdx // 조건 Column => 조건 값
            ),
            array (
				'SWFAQCategoryIdx'=> '=' // 조건 Column => Operator
            )
        );
		...
		try {
			...
			$this->jlamp_common_mdl->DBConnect('JLAMPBiz');
			// Table 설정
			$this->jlamp_common_mdl->setTable('SWFAQ');
		...
		if ($res) {
			...
			if ($res['rows']) {
				...
				if ($count >0) {
					...
					$fistBtn = '<li class="m-none"><a class="first"  title="처음" onclick="categoryClick(\''.$softwareFAQCategoryIdx.'\',\'1\')"><<</a></li>';
					$prevBtn = '<li><a class="prev"  title="이전" onclick="categoryClick(\''.$softwareFAQCategoryIdx.'\',\''.$prevBlockPage.'\')"><</a></li>';


					$endBtn = '<li class="m-none"><a class="last" title="마지막"  onclick="categoryClick(\''.$softwareFAQCategoryIdx.'\',\''.$endPage.'\')">>></a></li>';
					$nextBtn = ' <li><a class="next" title="다음"  onclick="categoryClick(\''.$softwareFAQCategoryIdx.'\',\''.$nextBlockPage.'\')">></a></li>';

					for ($i = $offsetPage; $i <= $endPage; $i++) {
						if ($i == $currPage)
							$numBtn .= '<li><a class="current" onclick="categoryClick(\''.$softwareFAQCategoryIdx.'\',\''.$i.'\')">'.$i.'</a></li>';
						else
							$numBtn .= '<li><a class="" onclick="categoryClick(\''.$softwareFAQCategoryIdx.'\',\''.$i.'\')">'.$i.'</a></li>';
					}
