...
class Qna extends JLAMP_Controller {
	...
	*/
	public function qnaList() {
		...
		$cssPart = array(
			'<link rel="stylesheet" href="/css/software/qna_list.css">'
		);
		$jsPart = array(
			'<script src="/js/software/qna_list.js"></script>'
		);
	...
	*/
	public function qnaRegister() {
		...
		$page = $this->jlamp_comm->xssInput('page', 'get');		// 페이지
		$cssPart = array(
			'<link rel="stylesheet" href="/css/software/qna_register.css">'
		);
		$jsPart = array(
			'<script src="/js/software/qna_register.js"></script>'
		);
	...
	*/
	public function qnaModify() {
		...
		$swQnAIdx = $this->jlamp_comm->xssInput('swQnAIdx', 'get');		// 권한이 없는 이전 페이지 경로
		$page = $this->jlamp_comm->xssInput('page', 'get');		// 페이지

		$cssPart = array(
			'<link rel="stylesheet" href="/css/software/qna_modify.css">'
		);
		$jsPart = array(
			'<script src="/js/software/qna_modify.js"></script>'
		);
	...
	*/
	public function qnaDetail() {
		...
		$cssPart = array(
			'<link rel="stylesheet" href="/css/software/qna_detail.css">'
		);
		$jsPart = array(
			'<script src="/js/software/qna_detail.js"></script>'
		);
	...
	public function qnaRegisterSave_prc() {
		...
		try {
			...
			// Table 설정
			$this->jlamp_common_mdl->setTable('SWQnA');
			...
			*/
			...
			for($i = 0; $i < count($_FILES['files']['name'])-1; $i++) {
				...
				if (isset($_FILES['files']['name'][$i])) {
					...
					$uploadData = $this->jlamp_upload->doUpload('files_'.$i, 'software/qna', true, false, 137, 176);
					...
					// Photo Data Setting
					$photoData = array (
						'SWQnAIdx' => $swQnAIdx, // 서비스 Q&A 일련번호
						'SavePath' => '/data/software/qna/'.$dateFolder.'/', // 저장경로
					...
					// Table 설정
					$this->jlamp_common_mdl->setTable('SWQnAFile');
	...
	*/
	public function qnaList_prc() {
		...
		$column = ' SWQnAIdx, SWQnACategoryIdx, UserIdx, StatusCode, Subject, Nickname, Content, AdminIdx, Reply, ReplyDT, ReadCount, RegDT,
		...
		$isArray = false;						   				  // 배열여부
		$sql='
		SELECT s.SWQnAIdx, s.SWQnACategoryIdx, s.UserIdx, s.StatusCode, s.Subject, s.Nickname, s.Content, s.AdminIdx, s.Reply, s.ReplyDT, s.ReadCount, s.RegDT,
		(SELECT u.Name FROM User u Where u.UserIdx = s.UserIdx Limit 1 ) AS Name,
		(SELECT c.CodeName FROM Code c Where c.Code = s.StatusCode Limit 1 ) AS CodeName FROM SWQnA s Where s.StatusCode != \'G0100004\'';
		*/
		$qnaHtml = '';
		$paginationHtml = '';
		$count = 0;
				...
				foreach ($res['rows'][0] as $key => $val) {
					$classColor = '';
						...
						<tr>
							...
							<td class="left">
								<a onclick="doDetailRows(\''.$val->SWQnAIdx.'\')" >
									<div class="tbl-qna">
	...
	*/
	public function qnaDetail_prc() {
		...
		$swQnAIdx = $this->jlamp_comm->xssInput ( 'swQnAIdx', 'get' ); // 서비스 Q&A 번호
		...
		(SELECT c.CodeName FROM Code c Where c.Code = s.StatusCode Limit 1 ) AS CodeName FROM SWQnA s Where s.SWQnAIdx='.$swQnAIdx;
		...
		$fileColumn = ' SWQnAFileIdx, SavePath, SaveFilename, RealFilename, Mime, FileSize '; // 검색할 Columns
		$fileFilter = array(
            array (
				'SWQnAIdx' => $swQnAIdx // 조건 Column => 조건 값
            ),
            array (
				'SWQnAIdx'=> '=' // 조건 Column => Operator
            )
        );
		...
		try {
			...
			$res = $this->jlamp_common_mdl->update('SWQnaIdx', $swQnAIdx, $data);
			/*
			if (!$res) {
				$result['returnCode'] = 'E001';
				$result['returnMsg'] = '정보관리 수정에 실패하였습니다.  \n관리자에게 문의바랍니다.';
	...
	*/
	public function qnaDelete_prc() {
		...
		$swQnAIdx = $this->jlamp_comm->xssInput ( 'swQnAIdx', 'post' ); // 서비스 Q&A 번호
	...
	*/
	public function qnaModifyDetail_prc() {
		...
		// SQL 변수
		$isArray = false; // 배열여부
		$column = 'SWQnAIdx, SWQnACategoryIdx, UserIdx, StatusCode, Subject, Nickname, Content '; // SELECT문
		// 조건절 설정
		$filter = array (
			array (
				'SWQnAIdx' => $swQnAIdx
		...
		if ($fileRes) {
			...
			if ($fileRes['rows']) {
                // 본문 List HTML 가공
				foreach ($fileRes['rows'][0] as $key => $val) {
					$detailImgHtml .='<div id="'.$val->SWQnAFileIdx.'">';
					$detailImgHtml .= $val->RealFilename;
					$detailImgHtml .= '<span style="color:#FF1D1D;cursor:pointer" onclick="doDeleteImg(\''.$val->SWQnAFileIdx.'\')">[삭제]</span>';
					$detailImgHtml .='</div>';
				}
	...
	public function qnaModifySave_prc() {
		...
		$subject = $this->jlamp_comm->xssInput('subject', 'post'); // 제목
		$content = $this->jlamp_comm->xssInput('txtarea_content', 'post'); // 내용
		$swQnAIdx = $this->jlamp_comm->xssInput('sw_qna_idx', 'post'); // 서비스  Q&A 
		...
		try {
			...
			for($i = 0 ; $i < count($deleteImgArray) ; $i++){
				...
				if ($deleteImgArray[$i]) {
					...
					$fileColumn = ' SWQnAFileIdx, SavePath, SaveFilename, RealFilename, Mime, FileSize  '; // SELECT문
					// 조건절 설정
					$fileFilter = array (
						array (
							'SWQnAFileIdx' => $deleteImgArray[$i]
						),
						array (
							'SWQnAFileIdx' => '='
						)
					...
					$res = $this->jlamp_common_mdl->delete('SWQnAFileIdx', $deleteImgArray[$i]);
				}
			}
